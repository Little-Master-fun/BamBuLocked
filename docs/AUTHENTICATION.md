...
首次通过学校认证的普通账号会在该工作站的 Printer 用户配置目录中建立本地认证缓存，保存输入账号、学校返回的姓名和学号，以及带独立随机盐的 PBKDF2-SHA256 密码校验值（600,000 次）。不保存明文密码、CAS 票据或认证响应。后续使用相同账号和密码时直接使用该缓存；本地尚无该账号才访问学校认证服务。缓存账号输入错误时不会把密码发送到学校认证服务。
